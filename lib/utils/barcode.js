...
function ean13(data) {
    if (data.length > 13)
        throw new Error("Data too long for EAN13");
    if (data.length < 12)
        data = data.padEnd(12, "0");
    if (/^\d+$/.test(data) === false)
        throw new Error("Invalid character in EAN13");
    // checksum
    let checksum = 0;
    for (let i = 0; i < 12; i++) {
    ...
    // Left Side
    const table_switch = ean13_table_switch_mask[data[0]];
    for (let i = 1; i < 7; i++) {
        const digit = data[i];
        const tab = table_switch[i - 1];
        const coding = ean13_bp[digit][tab];
        result.push(coding);
    }
...
const code128_bp = [
    ...
    { ascii: 36, code: "1000011100" }, // A: $, B: $, C: 04, BandCode: 121322
    ...
    { ascii: 41, code: "110001000" }, // A: ), B: ), C: 09, BandCode: 221213
    ...
    { ascii: 117, code: "10011110010" }, // A: DC4, B: u, C: 85, BandCode: 124211
...
function code128b(data) {
    ...
    if (data.length > 229)
        throw new Error("Data too long for Code128B");
    const result = [];
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (id === undefined)
            throw new Error("Invalid character in Code128B");
