...
import { findPrintTask, PrintTaskName, AbstractPrintTask, PrintOptions } from "../print_tasks";
...
export abstract class NiimbotAbstractClient extends EventEmitter<ClientEventMap> {
  ...
  public startHeartbeat(): void {
    ...
    this.heartbeatTimer = setInterval(() => {
      if (!this.isConnected()) return; // Skip if disconnected
      this.abstraction
        .heartbeat()
        .then((data) => {
          this.heartbeatFails = 0;
  ...
  public newPrintTaskAuto(options: PrintOptions): AbstractPrintTask | undefined {
    ...
    if (taskName) {
      return this.abstraction.newPrintTask(taskName, options);
    }
